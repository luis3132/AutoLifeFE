...
export async function POST(request) {
    ...
    const tecnicoMecanicaString = data.get('tecnicomecanica');
    const tecnicoMecanica = JSON.parse(tecnicoMecanicaString);

    const fs = require('fs');

    // Name of the folder
    const nombreCarpeta = tecnicoMecanica.vehiculo;

    // Verify if the folder exist
    const rutaCarpeta = path.join(process.cwd(), 'public', 'imagenes', 'Regulacion', 'TecnicoMecanica', nombreCarpeta);
    ...
    const folderName = tecnicoMecanica.vehiculo;
    const pathFile = path.join(process.cwd(), 'public/imagenes/Regulacion/TecnicoMecanica', folderName, file.name)
    ...
    // Set ID
    const id = tecnicoMecanica.id + '-' + anio + '-' + mes + '-' + dia + '-' + hora + '-' + min + '-' + seg
    const pathFoto = path.join('/imagenes/Regulacion/TecnicoMecanica', folderName, file.name)

    // Set the fotos object
    const foto = {
        id: id,
        foto: pathFoto,
        tecnicomecanica: tecnicoMecanica.id
    }
