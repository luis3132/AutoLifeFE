...
export async function POST(request) {
    ...
    const usuarioString = data.get('usuario');
    const usuario = JSON.parse(usuarioString);

    const fs = require('fs');

    // Name of the folder
    const nombreCarpeta = usuario.dni;

    // Verify if the folder exist
    const rutaCarpeta = path.join(process.cwd(), 'public', 'imagenes', 'Usuarios', nombreCarpeta);
    ...
    const folderName = usuario.dni;
    const pathFile = path.join(process.cwd(), 'public/imagenes/Usuario', folderName, file.name)
    ...
    // Set ID
    const id = usuario.dni + '-' + anio + '-' + mes + '-' + dia + '-' + hora + '-' + min + '-' + seg
    const pathFoto = path.join('/imagenes/Usuarios', folderName, file.name)
