...
export async function POST(request) {
    ...
    const soatString = data.get('soat');
    const soat = JSON.parse(soatString);

    const fs = require('fs');

    // Name of the folder
    const nombreCarpeta = soat.vehiculo;

    // Verify if the folder exist
    const rutaCarpeta = path.join(process.cwd(), 'public', 'imagenes', 'Regulacion', 'Soat', nombreCarpeta);
    ...
    const folderName = soat.vehiculo;
    const pathFile = path.join(process.cwd(), 'public/imagenes/Regulacion/Soat', folderName, file.name)
    ...
    // Set ID
    const id = soat.id + '-' + anio + '-' + mes + '-' + dia + '-' + hora + '-' + min + '-' + seg
    const pathFoto = path.join('/imagenes/Regulacion/Soat', folderName, file.name)
