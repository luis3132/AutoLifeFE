...
const NavbarUsuario: FC<NavbarUsuarioProps> = ({ logout }) => {
    ...
    return (
        <nav className="flex bg-amber-400 justify-between items-center w-full mx-auto">
            <div className="pl-[1%] ">
                <Link className="flex items-center hover:text-gray-500 cursor-pointer" href="/logged" replace>
            ...
            <div className="xl:static  absolute  min-h-fit left-0 top-[-100%] w-auto flex items-center px-5">
                <ul className=" xl:flex-row flex-col flex xl:items-center xl:gap-[4vw] gap-8 justify-center">
                    <li className="flex">
                        ...
                        <Link className="hover:text-gray-500" href="/logged/vehiculoPrivado" replace>Mis Vehiculos</Link>
                    </li>
                    <li className="flex">
                        <div className="p-1"><Icon icon="vaadin:car" /></div>
                        <Link className="hover:text-gray-500" href="/logged/vehiculoPublico" replace>Vehiculos</Link>
                    </li>
                    <li className="flex">
                        <div className="p-1"><Icon icon="fluent:vehicle-car-collision-24-regular" /></div>
                        <Link className="hover:text-gray-500" href="/logged/accidentePublico" replace>Accidentes</Link>
                    </li>
                    <li className="flex">
                        <div className="p-1"><Icon icon="fluent:vehicle-car-collision-24-regular" /></div>
                        <Link className="hover:text-gray-500" href="/logged/accidentePrivado" replace>Mis Accidentes</Link>
                    </li>
                    <li className="flex">
                        <div className="p-1"><Icon icon="mi:document" /></div>
                        <Link className="hover:text-gray-500" href="/logged/regulacion" replace>Regulacion</Link>
                    </li>
                    <li className="flex">
                        <div className="p-1"><Icon icon="material-symbols:home-repair-service-outline" /></div>
                        <Link className="hover:text-gray-500" href="/logged/servicios" replace>Servicios</Link>
                    </li>
                </ul>
            </div>
            <Transition
                ...
                <div className={`xl:hidden absolute bg-white min-h-[10%] left-0 top-[10%] max-sm:top-24 w-full flex items-center px-5 justify-center border-black border-2 transition-transform duration-300 ease-in-out ${navbar ? 'z-50 transform translate-y-0' : 'z-50 transform -translate-y-full'}`}>
                    <ul className="flex-col flex gap-8 justify-center items-center">
                        <li className="flex pt-2">
                            <div className="p-1"><Icon icon="vaadin:car" /></div>
                            <Link className="hover:text-gray-500" onClick={() => { setNavbar(!navbar); setIsShowing(!isShowing); }} replace href="/logged/vehiculoPrivado">Mis Vehiculos</Link>
                        </li>
                        <li className="flex">
                            <div className="p-1"><Icon icon="vaadin:car" /></div>
                            <Link className="hover:text-gray-500" onClick={() => { setNavbar(!navbar); setIsShowing(!isShowing); }} replace href="/logged/vehiculoPublico">Vehiculos</Link>
                        </li>
                        <li className="flex">
                            <div className="p-1"><Icon icon="fluent:vehicle-car-collision-24-regular" /></div>
                            <Link className="hover:text-gray-500" onClick={() => { setNavbar(!navbar); setIsShowing(!isShowing); }} replace href="/logged/accidentePublico">Accidentes</Link>
                        </li>
                        <li className="flex">
                            <div className="p-1"><Icon icon="fluent:vehicle-car-collision-24-regular" /></div>
                            <Link className="hover:text-gray-500" onClick={() => { setNavbar(!navbar); setIsShowing(!isShowing); }} replace href="/logged/accidentePrivado">Mis Accidentes</Link>
                        </li>
                        <li className="flex">
                            <div className="p-1"><Icon icon="mi:document" /></div>
                            <Link className="hover:text-gray-500" onClick={() => { setNavbar(!navbar); setIsShowing(!isShowing); }} replace href="/logged/regulacion">Regulacion</Link>
                        </li>
                        <li className="flex pb-2">
                            <div className="p-1"><Icon icon="material-symbols:home-repair-service-outline" /></div>
                            <Link className="hover:text-gray-500" onClick={() => { setNavbar(!navbar); setIsShowing(!isShowing); }} replace href="/logged/servicios">Servicios</Link>
                        </li>
                    </ul>
                </div>
            </Transition>
